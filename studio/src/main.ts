import "./styles.css";
import { buildDocumentGraphRagSnapshot, exampleSupportDocument } from "../../src/studio/document.js";
...
const palette = ["#8cc7ff", "#9be7b1", "#f3dc7a", "#e5a1c4", "#e88484", "#b9a7ff", "#85ddd4", "#e7b37a"];
...
const state = {
  ...
  focus: false,
  filter: "",
  retrievalQuery: "How does this repository organize GraphRAG storage, retrieval, Studio, and repo import?",
...
function sourceEntityId(relationship: GraphRelationship | Relationship): string {
  return typeof relationship.attributes?.sourceEntityId === "string" ? relationship.attributes.sourceEntityId : relationship.source;
}

function targetEntityId(relationship: GraphRelationship | Relationship): string {
  return typeof relationship.attributes?.targetEntityId === "string" ? relationship.attributes.targetEntityId : relationship.target;
}
...
function suggestedQueryFor(input: GraphRagSnapshot): string {
  const entityTitles = input.entities
    .slice()
    .sort((left, right) => Number(right.rank ?? right.degree ?? 0) - Number(left.rank ?? left.degree ?? 0))
    .slice(0, 3)
  ...
  return entityTitles[0] ? `What does this document say about ${entityTitles[0]}?` : "What is this document about?";
}
...
function buildGraph(input: GraphRagSnapshot): GraphModel {
  ...
  const reportsByCommunity = new Map(input.communityReports.map((report) => [report.community, report]));
  ...
  const entities: GraphEntity[] = input.entities.map((entity) => {
    const communityId = communityIdByEntityId.get(entity.id) ?? entity.communityIds?.[0] ?? "community-unclustered";
...
function buildDbTables(): DbTablePreview[] {
  ...
  return [
    dbTable("grag_documents", "core", documents.length, "Raw source documents plus metadata and source paths.", "Source reconstruction, citation display, incremental reindexing by document.", "Retrieval uses document ids and source metadata to cite where graph evidence came from.", [
    ...
    ], documents.map((document) => rowFromRecord(document, ["id", "title", "type", "text", "attributes"]))),
    dbTable("grag_text_units", "core", graph.textUnits.length, "Chunked grounding units used for local context and citations.", "Fast lexical/vector recall, bounded context windows, source-grounded answers.", "Basic/local retrieval scores these rows first, then follows links to entities and relationships.", [
    ...
    dbTable("grag_entities", "core", graph.entities.length, "Named concepts extracted from documents.", "Entity lookup, neighborhood expansion, explainable graph navigation.", "Local retrieval scores titles/descriptions, then expands through entity relationships and text-unit links.", [
    ...
    dbTable("grag_relationships", "core", graph.relationships.length, "Edges between extracted entities.", "Graph expansion, causality/dependency traversal, support impact analysis.", "Retrieval follows these edges from seed entities to pull nearby evidence and show why results connect.", [
    ...
    dbTable("grag_covariates", "core", covariates.length, "Claims, facts, or time-bound attributes attached to graph subjects.", "Temporal filters, claim verification, incident/status tracking.", "Advanced retrieval can filter or cite claims about an entity without mixing them into the entity record.", [
    ...
    ], covariates.map((covariate) => rowFromRecord(covariate, ["id", "covariateType", "subjectId", "status", "sourceText"]))),
    dbTable("grag_communities", "core", graph.communities.length, "Cluster records for groups of entities/relationships.", "Global search, topic summaries, high-level navigation.", "Global retrieval groups community reports by level/rank instead of scanning every chunk.", [
    ...
    dbTable("grag_community_reports", "core", snapshot.communityReports.length, "LLM/generated summaries of each community.", "Fast global answers, map-reduce summarization, analyst review.", "Global search loads these first, chunks them, maps over reports, then reduces partial answers.", [
    ...
    dbTable("grag_embeddings", "core", embeddings.length, "Vector records for documents, chunks, entities, relationships, or reports.", "Vector recall, pgvector/HNSW indexes, hybrid search.", "Basic search can load text_unit embeddings and compare them to the query vector before graph expansion.", [
    ...
    ], embeddings.map((embedding) => rowFromRecord(embedding, ["id", "targetKind", "targetId", "model", "dimensions", "text"]))),
    dbTable("grag_document_text_units", "link", documentTextUnits.length, "Ordered document-to-chunk links.", "Reconstructing document context and citation order.", "Lets retrieval climb from a hit chunk back to its source document.", linkColumns("document_id", "text_unit_id"), documentTextUnits),
    dbTable("grag_text_unit_entities", "link", textUnitEntities.length, "Which entities appear in each text unit.", "Grounded entity evidence and chunk-to-graph joins.", "When a chunk hits, retrieval can highlight entities mentioned in it.", linkColumns("text_unit_id", "entity_id"), textUnitEntities),
    dbTable("grag_text_unit_relationships", "link", textUnitRelationships.length, "Which relationships are grounded by each text unit.", "Edge citations and explainable relationships.", "When an edge is retrieved, the UI can show the exact grounding chunk.", linkColumns("text_unit_id", "relationship_id"), textUnitRelationships),
    dbTable("grag_entity_communities", "link", entityCommunities.length, "Entity membership in one or more communities.", "Community filtering and global search grouping.", "Lets global retrieval connect a selected entity to its report community.", linkColumns("entity_id", "community_id"), entityCommunities),
    dbTable("grag_entity_text_units", "link", entityTextUnits.length, "Entity-to-grounding-chunk links.", "Entity inspector sources and local context building.", "Local retrieval pulls source chunks for matching entities.", linkColumns("entity_id", "text_unit_id"), entityTextUnits),
    dbTable("grag_relationship_text_units", "link", relationshipTextUnits.length, "Relationship-to-grounding-chunk links.", "Relationship citations and edge confidence.", "Relationship hits cite the text units that justify the edge.", linkColumns("relationship_id", "text_unit_id"), relationshipTextUnits),
    dbTable("grag_community_entities", "link", communityEntities.length, "Community-to-entity membership.", "Cluster rendering and community summaries.", "The Studio and global search use this to traverse from a report to member entities.", linkColumns("community_id", "entity_id"), communityEntities),
    dbTable("grag_community_relationships", "link", communityRelationships.length, "Community-to-edge membership.", "Cluster edge summarization and local neighborhood views.", "Used to show the important relationships inside a selected community.", linkColumns("community_id", "relationship_id"), communityRelationships),
    dbTable("grag_community_text_units", "link", communityTextUnits.length, "Community-to-grounding-chunk membership.", "Community report source grounding.", "Global retrieval can cite chunks that support a report.", linkColumns("community_id", "text_unit_id"), communityTextUnits)
  ];
}
...
function bindEvents(): void {
  document.querySelector<HTMLInputElement>("#document-file")?.addEventListener("change", (event) => {
  ...
  document.querySelector<HTMLTextAreaElement>("#document-text")?.addEventListener("keydown", (event) => {
  ...
  document.querySelector<HTMLButtonElement>("#sample-reset")?.addEventListener("click", () => {
    void activateSnapshot(baseSnapshot, "sample", baseSourceTitle, "How does this repository organize GraphRAG storage, retrieval, Studio, and repo import?");
  });
  ...
  document.querySelector<HTMLFormElement>("#retrieval-form")?.addEventListener("submit", (event) => {
    event.preventDefault();
...
async function loadDocumentFile(file: File): Promise<void> {
  ...
  if (file.size > capabilities.maxUploadBytes) {
    setDocumentStatus(`File is too large. Max upload is ${Math.round(capabilities.maxUploadBytes / 1024 / 1024)}MB.`);
    return;
  }
  ...
  if (capabilities.openaiEnabled) {
    try {
      ...
      setDocumentStatus(`OpenAI extracted ${result.snapshot.entities.length} entities with ${result.model ?? capabilities.openaiModel}.`);
      return;
    } catch (error) {
  ...
  if (!textLike) {
    setDocumentStatus("This file type needs OPENAI_API_KEY on the Studio server. Text, Markdown, JSON, CSV, HTML, XML, and code files work locally.");
    return;
  }
...
async function buildDocumentGraphFromText(title: string, sourcePath: string, providedText?: string): Promise<void> {
  const textarea = document.querySelector<HTMLTextAreaElement>("#document-text");
...
function isTextLikeFile(file: File): boolean {
  ...
  return file.type.startsWith("text/") ||
    ...
    [".txt", ".md", ".mdx", ".json", ".csv", ".tsv", ".html", ".htm", ".xml", ".yaml", ".yml", ".log", ".js", ".ts", ".tsx", ".jsx", ".css", ".sql", ".py", ".rb", ".go", ".rs", ".java", ".c", ".cpp", ".h"].some((extension) => name.endsWith(extension));
}

async function uploadDocumentGraph(file: File, rawText?: string): Promise<DocumentGraphUploadResponse> {
  ...
  if (!body || typeof body !== "object" || Array.isArray(body) || !looksLikeSnapshot((body as Record<string, unknown>).snapshot)) {
  ...
  return {
    ...
    ...(Array.isArray(record.suggestedQueries)
      ? { suggestedQueries: record.suggestedQueries.filter((value): value is string => typeof value === "string") }
      : {})
  };
}
...
async function buildRepositoryGraph(repo: string, query: string): Promise<void> {
  ...
  const cleanQuery = query.trim() || "What are the important packages, plugins, storage surfaces, and auth flows in this repository?";
  ...
  setDocumentStatus(`Cloning and indexing ${cleanRepo}${useOpenAI ? " with OpenAI extraction" : " with local extraction"}...`);
  try {
    ...
    const mode = result.mode ? ` using ${result.mode}${result.model ? ` (${result.model})` : ""}` : "";
    setDocumentStatus(`Generated repo graph from ${fileCount} files${mode}: ${result.snapshot.entities.length} entities, ${result.snapshot.relationships.length} relationships.`);
    if (result.answer) {
...
async function uploadRepositoryGraph(repo: string, query: string, useOpenAI: boolean): Promise<RepoGraphResponse> {
  ...
  return {
    ...
    ...(record.stats && typeof record.stats === "object" && !Array.isArray(record.stats) ? { stats: record.stats as JsonObject } : {}),
    ...(Array.isArray(record.selectedFiles)
      ? {
          selectedFiles: record.selectedFiles.filter((entry): entry is { path: string; kind: string; bytes: number } =>
            Boolean(entry) &&
            typeof entry === "object" &&
            !Array.isArray(entry) &&
...
async function createUploadPayload(file: File, rawText?: string): Promise<DocumentGraphUploadPayload> {
...
function renderSourceState(): void {
  ...
  if (badge) {
    const prefix = state.snapshotSource === "document"
      ? "Document"
...
function entityMatchesFilter(entity: GraphEntity): boolean {
  ...
  return lexicalScore(queryTokens, [
    entity.title,
    entity.type,
    entity.description,
...
function rankedTextUnitHitEntities(hit: RetrievalHit, queryTokens: string[]): string[] {
  ...
  return hit.entityIds
    .flatMap((entityId) => {
      ...
      const lexical = lexicalScore(queryTokens, [
        entity.title,
        entity.type,
        entity.description,
        entity.communityTitle,
      ...
      const typeBoost = entity.type.includes("File") || entity.type.includes("Package") || entity.type.includes("Storage") ? 0.8 : 0;
      return [{
        id: entity.id,
        score: lexical * 12 + sourceMatch + typeBoost + entity.rank * 0.08 + entity.degree * 0.025
      }];
    })
...
function visibleSeeds(): Set<string> {
  ...
  for (const entity of graph.entities) {
    ...
    if (state.selectedCommunity && entity.communityId === state.selectedCommunity) ids.add(entity.id);
  }
  return ids;
}
...
function visibleIds(): Set<string> {
  ...
  for (const relationship of graph.relationships) {
    if (seeds.has(relationship.sourceId) || seeds.has(relationship.targetId)) {
      ...
      addScore(relationship.sourceId, 18 + relationship.weight * 2 + Number(source?.rank ?? 0) * 0.22);
      addScore(relationship.targetId, 18 + relationship.weight * 2 + Number(target?.rank ?? 0) * 0.22);
    }
  }
  const limit = state.selectedCommunity || state.filter ? MAX_FOCUSED_GRAPH_NODES + 48 : MAX_FOCUSED_GRAPH_NODES;
  return new Set(Array.from(scores.entries())
...
function computeLayout(): void {
  ...
  const nodes = graph.entities
    ...
    .map((entity) => ({ ...entity }));
  const groups = Array.from(new Set(nodes.map(groupFor))).sort((left, right) => left.localeCompare(right));
  ...
  nodes.forEach((node, index) => {
    ...
    node.vx = 0;
    node.vy = 0;
    node.r = Math.max(8, Math.min(23, 8 + Math.sqrt(node.degree || 1) * 2.8 + Math.sqrt(node.rank || 1) * 0.8));
    node.color = state.lens === "type" ? colorFor(node.type) : graph.communityById.get(node.communityId)?.color ?? colorFor(node.communityTitle);
  ...
  const links = graph.relationships
    ...
    .sort((left, right) => relationshipVisualScore(right, visibleSeedIds) - relationshipVisualScore(left, visibleSeedIds) || left.id.localeCompare(right.id))
    .slice(0, maxLinks);
...
function relationshipVisualScore(relationship: GraphRelationship, seeds: Set<string>): number {
  const selectedBoost = state.selectedId && (relationship.sourceId === state.selectedId || relationship.targetId === state.selectedId) ? 80 : 0;
  const seedBoost = seeds.has(relationship.sourceId) || seeds.has(relationship.targetId) ? 34 : 0;
  const hotPairBoost = seeds.has(relationship.sourceId) && seeds.has(relationship.targetId) ? 18 : 0;
...
function renderCommunities(): void {
  ...
  target.innerHTML = graph.communities
    .slice()
    .sort((left, right) => right.entityIds.length - left.entityIds.length || left.title.localeCompare(right.title))
    .map((community) => {
...
function renderGraph(): void {
  ...
  pattern.innerHTML = '<pattern id="grid" width="30" height="30" patternUnits="userSpaceOnUse"><path d="M 30 0 L 0 0 0 30" fill="none" stroke="#151515" stroke-width="1"/></pattern>';
  svg.appendChild(pattern);
  const viewport = document.createElementNS("http://www.w3.org/2000/svg", "g");
  viewport.setAttribute("class", "graph-viewport");
  viewport.setAttribute("transform", `translate(${state.view.panX} ${state.view.panY}) scale(${state.view.scale})`);
  ...
  for (const node of layout.nodes) {
    ...
    group.setAttribute("class", `node${isHot ? " hot" : ""}${isSelected ? " selected" : ""}${seeds.size > 0 && !isHot ? " dim" : ""}`);
...
function renderInspector(): void {
  ...
  if (!entity) {
    const largest = graph.communities.slice().sort((left, right) => right.entityIds.length - left.entityIds.length)[0];
    target.innerHTML = `
      <h3>Select a node</h3>
  ...
  const relationships = graph.relationships.filter((relationship) => relationship.sourceId === entity.id || relationship.targetId === entity.id);
  ...
  target.innerHTML = `
    ...
    <div class="section"><h2>Relationships</h2><div class="list">
      ${relationships.length ? relationships.slice(0, 8).map((relationship) => `<div class="relationship">${esc(relationship.description ?? `${relationship.source} -> ${relationship.target}`)}</div>`).join("") : `<p class="empty">No relationships in the current snapshot.</p>`}
    </div></div>
    <div class="section"><h2>Grounding</h2><div class="list">
      ${entity.sourcePaths.length ? entity.sourcePaths.slice(0, 8).map((path) => `<div class="source">${esc(path)}</div>`).join("") : `<p class="empty">No source paths attached.</p>`}
    </div></div>
    <div class="section"><h2>Text Units</h2><div class="list">
      ${textUnits.length ? textUnits.slice(0, 4).map((unit) => `<div class="source">${esc(unit.text)}</div>`).join("") : `<p class="empty">No linked text units.</p>`}
    </div></div>
...
function clientRetrieve(query: string): RetrievalResult {
  ...
  const entityHits = graph.entities
    .map((entity) => ({
      ...
      id: entity.id,
      title: entity.title,
      score: lexicalScore(queryTokens, [entity.title, entity.type, entity.description, entity.communityTitle, ...entity.sourcePaths].join(" ")) * 1.12,
  ...
  const relationshipHits = graph.relationships
    .map((relationship) => ({
      ...
      score: lexicalScore(queryTokens, [relationship.source, relationship.target, relationship.description ?? ""].join(" ")),
  ...
  const reportHits = graph.communities
    .flatMap((community) => {
      ...
      return [{
        ...
        score: lexicalScore(queryTokens, [community.title, community.report.summary ?? "", community.report.fullContent ?? ""].join(" ")) * 0.92,
  ...
  return {
    ...
    tokens: queryTokens,
    hits,
    context: hits.slice(0, 8).map((hit, index) => `${index + 1}. [${hit.kind}] ${hit.title} (${hit.score})\n${hit.text}`).join("\n\n"),
    stats: {
...
function renderRetrieval(): void {
  ...
  target.innerHTML = `
    ...
    ${retrieval.hits.slice(0, 5).map((hit) => `
...
async function loadCapabilities(): Promise<StudioCapabilities> {
  try {
    ...
    return {
      ...
      openaiModel: typeof body.openaiModel === "string" ? body.openaiModel : capabilities.openaiModel,
      maxUploadBytes: typeof body.maxUploadBytes === "number" ? body.maxUploadBytes : capabilities.maxUploadBytes
    };
  } catch {
    return capabilities;
  }
...
async function main(): Promise<void> {
  ...
  baseSourceTitle = window.__GRAG_STUDIO__?.title && window.__GRAG_STUDIO__?.title !== "GraphRAG Studio"
