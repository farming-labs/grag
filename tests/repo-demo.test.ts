...
describe("repository index API", () => {
  it("scans a repository and builds a queryable local graph", async () => {
    ...
    await writeFile(join(repo, "README.md"), "# Demo Repo\n\nThis repository has a GraphRAG docs engine and OpenAI indexing workflow.", "utf8");
    ...
    await writeFile(join(repo, "docs", "storage.md"), [
      "# Storage",
      "",
  ...
  it("clones remote repository sources with branch or tag selection", async () => {
    ...
    await runGit(repo, ["-c", "user.name=GRAG Test", "-c", "user.email=grag@example.com", "commit", "-m", "main"]);
    await runGit(repo, ["checkout", "-b", "docs"]);
    await mkdir(join(repo, "docs"), { recursive: true });
    await writeFile(join(repo, "docs", "github.md"), "# GitHub SaaS\n\nAsk AI indexes this branch.", "utf8");
    await runGit(repo, ["add", "."]);
    await runGit(repo, ["-c", "user.name=GRAG Test", "-c", "user.email=grag@example.com", "commit", "-m", "docs branch"]);
  ...
  it("indexes every eligible repository file by default", async () => {
    ...
    await writeFile(join(repo, "node_modules", "ignored", "package.json"), JSON.stringify({ name: "ignored" }), "utf8");
  ...
  it("balances large repo scans so docs and examples do not crowd out package source", async () => {
    ...
    await writeFile(join(repo, "package.json"), JSON.stringify({ name: "example-workspace" }), "utf8");
    await writeFile(join(repo, "packages", "cli", "src", "commands", "migrate.ts"), "export const migrate = 'run migrations';", "utf8");
    await writeFile(join(repo, "packages", "cli", "src", "generators", "prisma.ts"), "export const prisma = 'generate prisma schema';", "utf8");

    for (let index = 0; index < 80; index += 1) {
      await writeFile(join(repo, "docs", "content", "docs", `guide-${index}.mdx`), `# Guide ${index}\n\nAuth docs.`, "utf8");
      await writeFile(join(repo, "examples", "web", "src", `screen-${index}.tsx`), `export const Screen${index} = '${index}';`, "utf8");
      await writeFile(
    ...
    expect(paths.filter((path) => path.startsWith("packages/core/src/")).length).toBeGreaterThanOrEqual(16);
...
async function runGit(cwd: string, args: readonly string[]): Promise<void> {
  await new Promise<void>((resolveCommand, rejectCommand) => {
    ...
    child.once("close", (code) => {
      ...
      rejectCommand(new Error(`git ${args.join(" ")} failed with exit code ${code ?? "unknown"}.\n${Buffer.concat(stderr).toString("utf8").trim()}`));
    });
  });
}
